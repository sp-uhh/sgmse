...
base = "/export/home/jrichter/data/dns_chime3/"
chime_base = "/export/home/jrichter/data/CHiME3/"

# Clean speech for training
train_speech_files = sorted(glob(base + 'train/clean/*.wav', recursive=True))
...
noise_files = glob(chime_base + '/backgrounds/*.wav', recursive=True)
...
# Create target dir
train_noisy_path = Path(os.path.join(base, 'train/noisy/'))
...
for i, speech_file in enumerate(tqdm(train_speech_files)):
    ...
    id = speech_file.split('/')[-1].split('_')[0]
